# PlantScope — Polish Items

| # | Status | Item |
|---|--------|------|
| 1 | done | Dark theme & color scheme redesign — text is hard to read (gray plant name), do full design review |
| 2 | done | More space before "Save Plant" button |
| 3 | done | About section should NOT be collapsible — show full text |
| 4 | done | Move Ethnobotany notes above Photos (currently buried after long photo list) |
| 5 | done | Add sticky section nav/index (jump to Seasonality, Photos, Ethnobotany, etc.) |
| 6 | done | Fix Ethnobotany "Add note" form — currently not showing |
| 7 | done | Back button should preserve previously viewed taxon (browser history) |
| 8 | done | Restrict search to plants only (add taxon filter to API calls) |
